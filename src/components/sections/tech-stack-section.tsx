...
//           `, isHovered ? 'scale-150 shadow-3xl' : 'shadow-lg hover:shadow-xl'
//         )}
//         style={{
//           boxShadow: isHovered
 ...
 return (
  <section className="bg-background relative overflow-hidden">
    ...
    <div
      ...
      onMouseLeave={() => setIsPaused(false)}
    >
      {/* Optional: Slightly scale down for mobile to avoid overflow */}
      <div className="absolute inset-0 flex items-center justify-center scale-[0.85] sm:scale-100">
        {/* Central code icon */}
        <div className="w-16 sm:w-20 h-16 sm:h-20 bg-gradient-to-br from-card to-secondary rounded-full flex items-center justify-center z-10 relative shadow-2xl border border-border">
          <div className="absolute inset-0 rounded-full bg-primary/30 blur-xl animate-pulse"></div>
          <div
            className="absolute inset-0 rounded-full bg-accent/20 blur-2xl animate-pulse"
          ...
          <GlowingOrbitPath
            ...
            radius={
              typeof window !== 'undefined' && window.innerWidth < 640
                ? config.radius * 0.7 // smaller radius on mobile
                : config.radius
            }
            glowColor={config.glowColor}
            animationDelay={config.delay}
        ...
        {skillsConfig.map(config => {
          const adjustedRadius =
            typeof window !== 'undefined' && window.innerWidth < 640
              ? config.orbitRadius * 0.7
              : config.orbitRadius;

          const adjustedConfig = { ...config, orbitRadius: adjustedRadius };
          const angle = time * config.speed + (config.phaseShift || 0);
          return <OrbitingSkill key={config.id} config={adjustedConfig} angle={angle} />;
        })}
      </div>
    </div>
  </section>
